...
export class HomePage implements OnInit, AfterViewInit {
  ...
  otherAddress: string = 'Loading child address...'; // Update this line
  apiKey = 'e1ac02ccb68441bc97047ecc1f50a54c';  // API Key OpenCage
  ...
  async updateLocation() {
    try {
      ...
      this.http.post('https://5a0be9bb-9452-4905-b555-41be8697166c-00-31p974p2tm8vb.pike.replit.dev/', body, { headers }).subscribe(
        response => {
          console.log('Location updated on server:', response);
          this.getChildLocation(); // Get child location after updating parent location
        },
  ...
  async getChildLocation() {
    this.http.get('https://5a0be9bb-9452-4905-b555-41be8697166c-00-31p974p2tm8vb.pike.replit.dev/?role=child').subscribe(
      (response: any) => {
        const childLocation = response.child;
        console.log('Child location from server:', childLocation); // Log the child location data
        if (childLocation && this.map) {
          const lat = childLocation.latitude;
          const lon = childLocation.longitude;
          const address = childLocation.address;
          if (this.markerChild) {
            this.markerChild.setLatLng([lat, lon]);
          } else {
            this.markerChild = L.marker([lat, lon], {icon: this.getChildIcon()}).addTo(this.map);
          }
          this.otherAddress = address || 'Unknown Address'; // Update address text with child location
          console.log('Child location updated on map:', lat, lon, 'Address:', this.otherAddress);
        }
      },
      error => console.error('Error getting child location from server:', error)
    );
  }
